...
from Fobjetivo import Ploss # Función para construir inversa parcial
...
for k in range(0,n_itera):
    ...
    for p in range(0,pop):
        ...
        [Vm,Vcomp,Theta_n,_,Ssend,PQ,PV,Slack,Sflujo,PI,QI,Vgn,pij,pji,_] = PowerFlowAC(busa,linea,Gena,Vmin,Vmax)
        # Calculo de la función objettivo
        Fa = Ploss(Vm,Gen,busa,PI,QI,linea,Sflujo,PQ,PV,Slack,Vcomp,Vmin,Vmax,pij,pji)
    ...
    for p in range(0,pop):     
        ...
        # Calculo de la solucion de flujos AC
        [Vmag,Vcomp,Theta_n,_,Ssend,PQ,PV,Slack,Sflujo,Pin,Qin,Vgn,pij,pji,_] = PowerFlowAC(busb,lineb,Genb,Vmin,Vmax)
        # Calculo de la función objettivo
        Fb = Ploss(Vmag,Gen,busb,Pin,Qin,lineb,Sflujo,PQ,PV,Slack,Vcomp,Vmin,Vmax,pij,pji)
        # almacena valores de la función objetivo
        Obfunnew[k,p] = Fb
    ...
    print(k+1,"   ","Ploss (pu): ", "{:.8f}".format(bsof[k]),'  ',date.today() ,'  ',time.strftime("%X"))
...
# Calculo de la solucion de flujos AC
[Vmag,Vcomp,Theta_n,_,Ssend,PQ,PV,Slack,Sflujo,Pi,Qi,Vgn,pij,pji,_] = PowerFlowAC(busc,linec,Genc,Vmin,Vmax)
# función final
# Calculo de la función objettivo
Fc = Ploss(Vmag,Gen,busc,Pi,Qi,linec,Sflujo,PQ,PV,Slack,Vcomp,Vmin,Vmax,pij,pji)
